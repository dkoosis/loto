...
func TestDowngrade_ExclusiveToShared_FlipsRow(t *testing.T) {
	s := mustOpen(t)
	...
	// loto-zssw: downgrade is a row-mode change and nothing more.
	if fi, _ := os.Stat(rec.Target.Canonical); fi.Mode().Perm() != 0o644 {
		t.Errorf("downgrade must not touch mode bits; perm=%v", fi.Mode().Perm())
	}
}
...
func TestDowngradeLocks_Batch_MixedTargets(t *testing.T) {
	...
	if results[0].Err != nil {
		t.Errorf("excl: unexpected err=%v", results[0].Err)
	}
	...
	if results[1].Err != nil {
		t.Errorf("shared: unexpected err=%v", results[1].Err)
	}
