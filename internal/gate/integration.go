...
//
// ‡ Promotion is what ADVANCES this ref once a candidate is accepted — Promote
// (promotion.go), reached from `loto promote`. Until a repo runs its first
// promotion, every submit validates against the SAME frozen point this
// bootstrap picks — which is correct, not a workaround: "integration" is
// meant to be a stable reference point, and a repo with no promotion run yet
// legitimately has no integrated state beyond "wherever HEAD was when the
// first candidate was submitted."
//
