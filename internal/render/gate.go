...
// same input, byte-identical output), independent of caller order. A
// claim-kind row gets no fix command — no takeover verb exists — and
// carries the options line instead; a lock-kind row carries the existing
// `loto unlock --force` fix block, mirroring printCheckConflicts.
func EmitGateDeny(w io.Writer, rows []GateDenyRow) {
	...
	for i := range sorted {
		r := &sorted[i]
		fmt.Fprintf(w, "✗ path=%s kind=%s holder=%s intent=%q expires_at=%s\n",
