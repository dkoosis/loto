...
func TestEmitLLMDashboardEventHeld(t *testing.T) {
	...
	got := buf.String()
	for _, want := range []string{"held", "BraveOtter", "store.go", "intent:edit store", "ts:2026-04-30T14:32:00Z"} {
...
func TestEmitLLMDashboardEventMsg(t *testing.T) {
	...
	got := buf.String()
	for _, want := range []string{"msg", "from:AmberFalcon", "to:BraveOtter", "need 5min"} {
		if !strings.Contains(got, want) {
			t.Errorf("output %q missing %q", got, want)
		}
	}
...
func TestEmitHumanDashboardEventReserved(t *testing.T) {
	...
	got := buf.String()
	for _, want := range []string{"reserved", "BraveOtter", "internal/store/**", "scan"} {
		if !strings.Contains(got, want) {
			t.Errorf("output %q missing %q", got, want)
		}
	}
