...
func TestLookupEntryFoldPrefersExactOverFolded(t *testing.T) {
	...
	got, ok := lookupEntryFold(dir, "Store")
	if !ok || got != "Store" {
	...
	if _, ok := lookupEntryFold(dir, "absent"); ok {
		t.Error("lookupEntryFold reported a hit for an entry that does not exist")
	}
	if _, ok := lookupEntryFold(filepath.Join(dir, "no-such-dir"), "x"); ok {
...
func TestResolveDiskCaseFoldsDirectorySpelling(t *testing.T) {
	...
	got := resolveDiskCase(repo, tcStoreVariantGo)
...
func TestResolveDiskCaseKeepsUnknownTail(t *testing.T) {
	...
	got := resolveDiskCase(repo, "internal/Store/NotYetWritten.go")
...
func TestResolveDiskCaseLeavesUnwalkableShapes(t *testing.T) {
	...
	for _, c := range cases {
		if got := resolveDiskCase(c.repoTop, c.in); got != c.in {
