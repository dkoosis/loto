...
// DIFFERENT process is still a peer and still protects its tree. A child
// spawned for the test stands in for that process — genuinely live, and
// provably not this one on any host.
...
// TestLiveOwnerUUIDsInRepo_UnknownProcStartCollapsesNothing is the PR #351
// review's first finding: 0 is ProcStart's unknown sentinel, so two records
// on one pid with no readable start-time on either side are two unknowns, not
// one process. They stay in |S| — over-counting refuses a ref update, and
// that is the direction that cannot cost anyone a working tree.
func TestLiveOwnerUUIDsInRepo_UnknownProcStartCollapsesNothing(t *testing.T) {
	clearIdentityEnv(t)
	stubProcStart(t, 0, false) // no readable start-time, at record and at sweep
	t.Setenv("LOTO_PID", strconv.Itoa(os.Getpid()))
	t.Setenv("CLAUDE_CODE_MESSAGING_SOCKET", existingSocket(t))
	const repo = "/repos/a"
	...
	live := LiveOwnerUUIDsInRepo(repo)
	if len(live) != 2 {
		t.Fatalf("|S| = %d, want 2 — equal pids with two unknown start-times are not one process: %v", len(live), live)
	}
	if _, ok := live["owner-precleared"]; !ok {
		t.Errorf("the record that cannot be proven same-process must stay in S: %v", live)
	}
}
