...
// an identity via resolveSubagent. A stamp pins only when a session id is
// present to derive from; an empty or malformed (traversal-shaped) stamp falls
// open to normal resolution. Callers deciding fail-open vs fail-closed
// (`loto check --gate`) must not treat such an id as pinned.
func SubagentIDPins(id string) bool {
	if id == "" || os.Getenv("CLAUDE_CODE_SESSION_ID") == "" {
		return false
	}
	_, err := sessionPath(id)
	return err == nil
}
...
// normal resolution.
//
// A /team subagent inherits the parent's session id, collapsing every sibling
...
// root — into LOTO_SUBAGENT_ID, and the owner is derived from (session id,
// stamp) so siblings get distinct owners the existing conflict logic
// serializes, with no cache file to race for.
//
...
func resolveSubagent() (*Agent, bool) {
	...
	host, _ := HostID()
	return &Agent{UUID: deriveUUID(os.Getenv("CLAUDE_CODE_SESSION_ID"), sub), Host: host}, true
}
