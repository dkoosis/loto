...
tag layer and the harness gate; foreground file-flock and global flock
remain on the roadmap. Filesystem enforcement was shipped and then
**retired** (loto-zssw) — see below.
...
(`mode | 0200`). The trouble is that the holder edits through the same
filesystem as everyone else: the strip locked out the agent whose whole
operating loop is lock → edit → unlock, with no "between" to edit in. On
2026-08-12 a holder's own `>` redirect failed silently inside a `&&`
chain and a stale `NORTH_STAR.md` body reached main. Files were also
observed orphaned at `0444` with no row to explain them, after a session
died mid-hold.
...
stragglers; it drains itself as the audit trail rotates.
