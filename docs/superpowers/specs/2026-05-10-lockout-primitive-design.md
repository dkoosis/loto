...
Round-trips `0644`, `0664`, `0600`, `0640` for owner-write recovery. Group/other write bits are not preserved across a lock cycle — rare on a single-user dev box, recoverable with `chmod g+w` after. The simplification (no `original_mode` column, no migration, no stat-then-chmod race) is worth the lossy round-trip.
...
1. `flock(LOCK_EX)` the project op-flock `<state>/projects/<slug>/lock-op.flock` (blocking — `loto lock` is fast; contention is acceptable).
2. Begin SQL transaction.
3. Reject any non-existent or non-regular-file target before any chmod — clear error, zero side effects on disk.
4. Collect blockers (existing logic). Any → release op-flock, return `ConflictError` JSON.
5. For each target in canonical-sorted order: `stat()`, then `chmod(mode &^ 0222)`. If any chmod fails → restore (`chmod(mode | 0200)`) on already-chmodded targets; if restore also fails → emit `mode_restore_failed` system tag, exit 3, suggest `loto doctor`. Otherwise abort tx, release op-flock, return error.
6. Insert lock rows (no `original_mode` column).
7. Commit tx, release op-flock.
8. Output (per `design.md`):
   ```
   ✓ locked count=3
   ✓ target=a.go
   ✓ target=b.go
